...
class ErrorBoundaryHandler extends _react.default.Component {
    static #_ = this.contextType = _approutercontextsharedruntime.AppRouterContext;
    constructor(props){
...
async function createFetch(url, headers, fetchPriority, shouldImmediatelyDecode, signal) {
    ...
    let fetchPromise = processed.then((param)=>{
        let { response } = param;
        return response;
    });
    ...
    const rscResponse = {
        ...
        cacheData: processed.then((param)=>{
            let { cacheData } = param;
            return cacheData;
        })
    };
...
function createCacheNode(rsc, prefetchRsc, head, prefetchHead) {
    let scrollRef = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : null;
    return {
        rsc,
        prefetchRsc,
        head,
...
function permanentRedirect(/** The URL to redirect to */ url) {
    let type = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'replace';
...
async function fetchServerAction(state, nextUrl, param) {
    ...
    if (isRscResponse) {
        ...
        const responsePromise = redirectLocation ? (0, _fetchserverresponse.processFetch)(res).then((param)=>{
            let { response: r } = param;
            return r;
        }) : Promise.resolve(res);
...
function serverActionReducer(state, action) {
    ...
    return fetchServerAction(state, nextUrl, action).then(async (param)=>{
        let { revalidationKind, actionResult, actionFlightData: flightData, actionFlightDataRenderedSearch: flightDataRenderedSearch, redirectLocation, redirectType, isPrerender, couldBeIntercepted } = param;
...
function createInitialRouterState(param) {
    let { navigatedAt, initialRSCPayload, initialFlightStreamForCache, location } = param;
